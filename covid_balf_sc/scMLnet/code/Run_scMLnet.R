...
#output:GeneList
getHighExpGene <- function(GCMat,barCluTable,CluNum.1,CluNum.2,pval,logfc)
{
  ...
  getpval <- function(gene){
    result <- t.test(x = data.test[gene, cells.1], y = data.test[gene, cells.2])
  ...
  data.test <- data.test[,c(cells.1,cells.2)]
...
#test data
CreateMulNet <- function(GCMat, BarCluFile, RecClu, LigClu,
                         ...
                         RecTFLib = "./database/RecTF.txt")
{
  ...
  #getHighExpGene
  RecClus <- getHighExpGene(GCMat,BarCluTable,RecClu,LigClu,pval,logfc)
  LigClus <- getHighExpGene(GCMat,BarCluTable,LigClu,RecClu,pval,logfc)
